...
parser.add_argument("--data_folder", default="../nerf_pretrain/data_snrs")
...
# np.save("/home/yigit/Metaworld/NERF_Dataset/focal", focal)
...
# poses = np.tile(poses, (5,1,1))
# poses = poses[:247]
# print(poses.shape)
print(env.camera_name)
# nerf_config_yaz(poses)
...
images = np.zeros((120 * 120, 480, 480, 3))
    ...
    for t in range(n_timesteps * timestep_freq):
        ...
        if image_save and t % timestep_freq == 0:
            for camId in range(n_cams):
                ...
                os.mkdir(
                    os.path.join(data_folder, task_name, f"{ep * n_timesteps + t:06d}")
                )
                os.mkdir(
                    os.path.join(
                        data_folder, task_name, f"{ep * n_timesteps + t:06d}", "rgb"
                    )
                )

                with open(
                    os.path.join(data_folder, task_name, "intrinsics.txt")
                ) as file:
                    f = 0.5 * 480 / np.tan(np.pi / 8)
                    intrinsic_str = f"{f} 240.0 240.0 0.\n0. 0. 0.\n1.\.480 480"
                ...
                if (
                    ...
                    pixels = 255 * geom_ids
                    seg = Image.fromarray(pixels.astype(np.uint8))
                    """
                    seg.save(
...
# np.save("/home/yigit/Metaworld/NERF_Dataset/images", images)
