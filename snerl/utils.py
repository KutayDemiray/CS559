...
class ReplayBuffer(Dataset):
    ...
    def add(self, obs, action, reward, next_obs, done, episode_step):
        # print("obses", self.obses.shape) # buffer_size x 9 x 128 x 128
    ...
    def sample_cpc(self):
        ...
        if self.multicam_contrastive:
            obses = sample_view_from_multiview(
                obses, self.multiview, rl=True, frame_stack=self.frame_stack
            )
            next_obses = sample_view_from_multiview(
                next_obses, self.multiview, rl=True, frame_stack=self.frame_stack
            )
            pos = sample_view_from_multiview(
                pos, self.multiview, rl=False, frame_stack=self.frame_stack
            )
...
def random_crop(imgs, output_size):
    ...
    cropped_imgs = transforms.Lambda(
