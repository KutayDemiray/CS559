...
class SegnetDecoder(nn.Module):
    ...
    def forward(self, x, inds, sizes):
        ind1, ind2, ind3, ind4, ind5 = inds
...
class SegNet(nn.Module):
    def __init__(self, in_chn=3, out_chn=32, BN_momentum=0.5):
        ...
        self.encoder = SegnetEncoder(
            in_chn=in_chn, out_chn=out_chn, BN_momentum=BN_momentum
        )
    ...
    def forward(self, x):
        x, inds, sizes = self.encoder(x)
        x = self.decoder(x, inds, sizes)
        return x
