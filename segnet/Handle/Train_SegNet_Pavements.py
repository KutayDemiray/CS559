...
def main(args):
    ...
    assert (
        len(model_json["cross_entropy_loss_weights"]) == model_json["out_chn"]
    ...
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=model_json["batch_size"], shuffle=True, num_workers=4
    )
        ...
        if os.path.isfile(weight_fn):
            ...
            print(
                "Loaded checkpoint '{}' (epoch {})".format(
                    weight_fn, checkpoint["epoch"]
                )
            )
        else:
            print(
                "No checkpoint found at '{}'. Will create new checkpoint.".format(
                    weight_fn
                )
            )
    else:
    ...
    for i in range(epoch + 1 if epoch is not None else 1, run_epoch + 1):
        ...
        loss_total.append((sum_loss / (j * trainloader.batch_size)))
    ...
    torch.save(model.encoder.state_dict(), f"encoders/aff_encoder_{run_epoch}.pt")
    np.save("/home/yigit/SegNet_PyTorch/Handle/LossCurve", np.array(loss_total))
...
if __name__ == "__main__":
    ...
    # FORMAT DIRECTORIES
    parser.add_argument(
        "pavements_raw_dir", type=str, help="Directory: Pavements raw training images"
    )
    parser.add_argument(
        "pavements_labelled_dir",
        type=str,
        help="Directory: Pavements annotated training images",
    )
    # parser.add_argument("tensorboard_logs_dir", type=str, help="Directory: Logs for tensorboard")
    parser.add_argument(
        "--weight-fn", type=str, help="Path: Trained weights", default=None
    )
